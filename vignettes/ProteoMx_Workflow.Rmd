...
# For this tutorial, we will use 'tiny_geomx' as our primary object
geomx_set <- tiny_geomx
...
print("q_norm" %in% names(assayData(geomx_set)))
```
...
fit_results <- GetMixModelResults(geomx_set)
...
PlotMixModel(geomx_set, protein = target_protein)

# Plot with forced parameters (e.g., 3 components, Equal Variance)
PlotMixModel(geomx_set, protein = target_protein, ncomp = 3, ev = TRUE)
```
...
print(paste("Original Features:", nrow(geomx_set)))
