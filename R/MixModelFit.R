...
#' @examples
#' \dontrun{
#'   # Fit up to 3 components
#'   geomx_set <- MixModelFit(geomx_set, ncomps = 3)
#' }
MixModelFit <- function(geomx_set, ncomps = 3) {
  ...
  for (ev_flag in ev_values) {
    for (n in 1:ncomps) {
      ...
      # Initialize with NULLs. This sets length to exactly 580.
      ...
      for (i in seq_along(all_proteins)) {
        ...
        # --- THE FIX ---
        # Only assign if fit is NOT NULL. 
        # If we assign NULL to a list index (e.g. list[[i]] <- NULL), R DELETES that index.
        # By skipping assignment, we leave it as the initialized NULL placeholder.
        if (!is.null(fit)) {
          protein_fits[[i]] <- fit
        }
      }
      ...
      # Now lengths match perfectly (580 vs 580)
      fits_tbl <- tibble(
